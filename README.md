...
This project was built in [Cursor](https://cursor.com) with Agent (**vibe coding**): prompts in the IDE, not a handwritten codebase. Local Agent transcripts from 8–16 Sep 2026 add up to about **170 million tokens** (102 user prompts, 1,343 model steps, counting conversation context replayed on each step). That is an estimate from this machine’s session logs, not the Cursor billing dashboard.

中文：本项目通过 Cursor 的 vibe coding（Agent）模式开发。2026 年 9 月 8 日至 16 日本机 Agent 会话合计约 **1.7 亿 token**（102 次提问、1343 次模型步骤，按每一步回放对话上下文估算）。
